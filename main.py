...
flags.DEFINE_float("firing_rate", 0.1, "maximum firing rate of the simulated responses")
flags.DEFINE_boolean("visualize_data", True, "True for visualizing data [True]")
flags.DEFINE_string("neuron", "", "file corresponding to the neuron from Kayser's data to be analyzed")
FLAGS = flags.FLAGS
...
def main(_):
  ...
  if FLAGS.neuron!='':
      FLAGS.checkpoint_dir = FLAGS.checkpoint_dir + FLAGS.neuron + '_num_bins_' + str(FLAGS.num_bins)  + '_iteration_' + FLAGS.iteration
      
      FLAGS.sample_dir = FLAGS.sample_dir + FLAGS.neuron + '_num_bins_' + str(FLAGS.num_bins)  + '_iteration_' + FLAGS.iteration
  else:
