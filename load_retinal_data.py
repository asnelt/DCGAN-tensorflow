...
def load_spikes(folder, movie, bin_size, num_bins):
    #folder = '/home/manuel/DATA/data/Scales/RetinalData/NeuralData/Spikes/'
            ...
            for ind_w in range(num_trials_per_movie_repetition):
                ...
                if window_spks.size !=0:
                    ...
                    for ind_spks in range(num_spks):
                        binned_mat[ind_neuron][contador][int(np.ceil((window_spks[ind_spks]-1)/bin_size))-1] = binned_mat[ind_neuron][ind_trial][int(np.ceil((window_spks[ind_spks]-1)/bin_size))-1] + 1
                contador += 1    
